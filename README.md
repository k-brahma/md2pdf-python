...
CLI・GUIのログは、プロジェクト内の `logs/pdf_converter.log` に保存されます。
ログには入力・出力パス、ファイルごとの成功・失敗、ページ数、ファイルサイズ、
処理時間、例外のスタックトレースが記録されます。GUIではINFO以上のログを
画面下部のログ欄でもリアルタイムに確認できます。

ログファイルは5MBごとにローテーションし、過去5世代を保持します。

PDFは同じ出力ディレクトリの一時ファイルへ生成し、PDFとして読み込めることを
確認してから既存PDFと置き換えます。変換に失敗した場合、以前のPDFは残ります。
